...

{
    ...
    {
        p1 =       (k_S6_S7 * dt);     // (first order) uniimolecular   forward transition to *E'-P.Ca2                 [S7]
    ...
    {
        p1 =      (k_S7_S9              * dt); // (first order)         unimolecular  forward transition to *E'-P.Ca            [S9]
        p2 = p1 + (k_S7_S6 * MgADP_conc * dt); // (pseudo-first order)   bimolecular backward transition to *E'-P.ADP.Ca2       [S6]
        p3 = p2 + (k_S7_S8              * dt); // (first order)         unimolecular backward transition to  E'~P.Ca2           [S8]
        if(randNum < p1) //If the random number is less than first probability , then transition to next state
        {
            state = 9; //forward transition to *E'-P.Ca         [S9]
        }
        else if(randNum < p2)
        {
    ...
    else if(current_state == 8)
    {
        ...
        p2 = p1 +  (k_S8_S5 * MgADP_conc * dt);     // (pseudo-first order)  bimolecular backward transition to E'~P.ADP.Ca2                   [S5]
        if(randNum < p1) //If the random number is less than first probability , then transition to next state
        {
            state = 7;   // forward transition to *E'-P.Ca2      [S7]
        }
    ...
    //-----------------------------------------------------------------------------------------------------------------------
    //
    // if(state  = 9): Then      S7 [*E'-P.Ca2] <-- S9 {*E'-P.Ca} --> S10 [*E-P.Ca] +Ca   else stay as *E'-P.Ca
    //
    ...
    {
        p1 =       (k_S9_S10 			   * dt);     // (first-order)         unimolecular  forward transition to *E-P.Ca    [S10]
        p2 = p1 +  (k_S9_S7  * Ca_sr_conc  * dt);     // (pseudo-first order)   bimolecular backward transition to *E'-P.Ca2  [S7]
        if(randNum < p1) //If the random number is less than first probability , then transition to next state
        {
    ...
    //-----------------------------------------------------------------------------------------------------------------------
    //
    // if(state  = 10): Then   S9 [*E'-P.Ca] <-- S10 {*E-P.Ca} --> S11 [*E-P] +Ca   else stay as *E-P.Ca
    //
    ...
    else if(current_state == 10)
    {
        ...
        p2 =       (k_S10_S9  * dt);     // (first-order)  bimolecular backward transition to *E'-P.Ca    [S9]
        if(randNum < p1) //If the random number is less than first probability , then transition to next state
        {
        ...
        {
            state = 9; // backward transition to   *E'-P.Ca     [S9]
        }
    } //if it is not greater than either probability  then stay in the same state
    ...
    else if(current_state ==12)
    {
        ...
        p2 =       (k_S12_S11 * dt);    // (first order) bimolecular backward transition to *E-P                     [S11]
        if(randNum < p1) //If the random number is less than first probability , then transition to next state
        {
